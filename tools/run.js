...
var startProxy = function (outerPort, innerPort, callback) {
  ...
  var server = http.createServer(function (req, res) {
    ...
    var proxyIt = function () {
      proxy.web(req, res, {target: 'http://'+process.env.IP+':' + innerPort});
    };
    if (Status.listening) {
  ...
  server.on('upgrade', function(req, socket, head) {
    var proxyIt = function () {
      proxy.ws(req, socket, head, { target: 'http://'+process.env.IP+':' + innerPort});
    };
    if (Status.listening) {
      // server is listening. things are hunky dory!
      proxyIt();
  ...
  server.listen(outerPort, process.env.IP, callback);
};
...
exports.run = function (context, options) {
	var outerPort = options.port || process.env.PORT;
	 var innerPort = 16010;
	var mongoPort = 16000;
	var testPort = 16011;
  ...
  ("mongodb://" + process.env.IP + ":" + mongoPort + "/meteor");
  var test_mongo_url = "mongodb://"+process.env.IP+":" + mongoPort + "/meteor_test";
  var firstRun = true;
